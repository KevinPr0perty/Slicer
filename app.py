...
def split_excel_to_xlsx_bytes(
    ...
    for i in range(parts)
        ...
        # Save this part to bytes
        bio = io.BytesIO()
        out_wb.save(bio)
        bio.seek(0)

        outputs.append((fpart_{i+1}_of_{parts}.xlsx, bio.read()))
...
st.title(Temu Excel Splitter (999 rows per file))

st.write(
    Upload your Temu inventory template Excel. 
    This will output a ZIP with multiple .xlsx files, each keeping the first two rows the same.
)
...
max_col_override = st.number_input(
    Max columns to copy (optional, 0 = auto),
    min_value=0,
    max_value=500,
    value=0,
    step=1
)
max_col_override = None if max_col_override == 0 else int(max_col_override)

if uploaded
    st.success(fLoaded {uploaded.name})

    if st.button(Split & Create ZIP)
        try
            ...
            st.success(fDone! Created {len(parts)} file(s).)
            st.download_button(
                label=Download ZIP,
                data=zip_bytes,
